...
func (inst *innerContextGetter) GetPropertyString(name string, _default string) string {
	return inst.context.GetProperties().GetProperty(name, _default)
}

func (inst *innerContextGetter) GetPropertyInt(name string, _default int) int {
	str, err := inst.context.GetProperties().GetPropertyRequired(name)
	if err != nil {
		return _default
	}
