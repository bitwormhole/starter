...
func LoadEmbedResources(fs *embed.FS, path string) Resources {
	...
	if err != nil {
		vlog.Warn("LoadResourcesFromEmbedFS error: " + err.Error())
...
func (inst *simpleEmbedRes) _Impl() Res {
	return inst
}
...
func (inst *simpleEmbedTreeLoader) load(fs *embed.FS, path string) (Resources, error) {

	table := make(map[string]Res)
...
func (inst *simpleEmbedTreeLoader) walkDir(fs *embed.FS, pathFs string, pathRes string, depthLimit int, dst map[string]Res) error {
	if depthLimit < 1 {
...
func (inst *simpleEmbedTreeLoader) onFile(fs *embed.FS, pathFs string, pathRes string, dst map[string]Res) {
	item := &simpleEmbedRes{}
	item.src = pathFs
	item.fs = fs
	item.isdir = false
